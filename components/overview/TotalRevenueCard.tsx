...
function TotalRevenueCard({ orders }: TotalRevenueCardProps) {
  ...
  useEffect(() => {
    ...
    const revenue = filteredOrders.reduce((acc, curr) => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <h1 className="text-2xl font-bold">{formatter.format(totalRevenue)}</h1>
      </CardContent>
    </Card>
  );
}
